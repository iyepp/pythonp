...
def mySendRecv(myfile_list, q, sleepcount):
    ...
    for i, f in enumerate(file_list):
		...
		# 아래 스트링은 필터로 거른다
            ...
            # temporary time for debugging
            print("++++++++++++++++++++++++++++++++++++++++++++++++++")
            # 파일에 쓴다
            delayedfile = open(delay_file, 'a')
            ...
            if ret_data == b'}\r\n':
                ...
                # 참이고, 리턴 값의 끝이 아니면, 화면에 출력한다.  그리고 에러가 있는지 파싱해 본다.
                    ...
                    if ret_data.upper().find(b'ERROR')  != -1: # 뭔 가가 있으면 # 대소문자 구분 필요
                        ...
                        # Logging the Error log in the local file
                        myfile = open(error_file, 'a')
                    ...
                    elif ret_data.upper().find(b"KILL_PROCESS") != -1:
                        print("\t\t[myMonitoring] Processed was killed by KILL_PROCESS msg !!!!!")
                        print("[Kill Log]", ret_data)
                        # Logging the Error log in the local file
                        errorfile = open(error_file, 'a')
                        
                        while not q.empty():
                            errorfile.write(ret_data)
                            ret_data = q.get()

                        errorfile.flush()
                        errorfile.close()
                        
                        # broadcasting the message of "KILL_PROCESS"
                        q.urgent(str("KILL_PROCESS"))
                        ...
                        # Logging the Success log in the local file
                        successfile = open(success_file, 'a')
